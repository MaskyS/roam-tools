...
> **Scope:** this is an inward-facing architecture doc. It deliberately describes only **core's public contract** and treats the hosted server abstractly — none of the hosted server's backend internals live here. For version-bump mechanics and the partial-publish audit, see `CLAUDE.md`; this doc does not duplicate them.
...
- Introduces a caret/tilde dep range in `mcp`'s or `cli`'s `package.json` for a sibling `@roam-research/*` package — `bump-version.mjs` writes **exact** pins on purpose (see the partial-publish invariant in `CLAUDE.md`).
